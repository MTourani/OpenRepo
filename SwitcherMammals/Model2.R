...
nmodel <- nimbleCode( { 
  ...
  mu.betalpsiFor ~ dnorm(0, 0.001) # For LC effect
  ...
  for (k in 1:nspec) {
    ...
    for (i in 1:nsite[k]) {
      ...
      logit(psi[i, k]) <- lpsi[k] + betalpsiYr[k] * Xyear[siteID[i, k], 2] + betalpsiFor[k] * lc[siteID[i, k], 2] + betalpsiClim[k] * clim[siteID[i, k], 3] + betalpsiI[k]*lc[siteID[i, k], 2]*clim[siteID[i, k], 3] + cint.psi[hs[i, k], k]  
...
mcmcConf <- configureMCMC(model, monitors = c("mu.lpsi", "sd.lpsi", "mu.lp", "sd.lp", "mu.betalpsiI", "sd.betalpsiI"))
